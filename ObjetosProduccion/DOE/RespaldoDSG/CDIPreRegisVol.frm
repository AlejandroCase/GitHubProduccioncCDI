...
FiltroTipo=General
RefrescarAlEntrar=S
FiltroGeneral=CDIPreRegisDet.Estatus NOT IN (<T>CANCELADO<T>,<T>CONCLUIDO<T>)<BR>and CDIPreRegisDet.FechaEmision >=<T>20150101<T><BR>AND (CDIPreRegisDet.CDISocio=<T>{Info.CDISocio}<T> or CDIPreRegisDet.Cliente=<T>{Info.ClienteA}<T>)
