
[Forma]
Clave=CDIVMemGest
Icono=0
CarpetaPrincipal=CDIVMemGest
Modulos=(Todos)
Nombre=Observaciones

ListaCarpetas=CDIVMemGest
BarraHerramientas=S
AccionesTamanoBoton=15x5
AccionesDerecha=S
ListaAcciones=Cerrar
PosicionInicialIzquierda=515
PosicionInicialArriba=261
PosicionInicialAlturaCliente=169
PosicionInicialAncho=307
PosicionSec1=42
VentanaTipoMarco=Normal
VentanaPosicionInicial=Centrado
VentanaEscCerrar=S
VentanaEstadoInicial=Normal
VentanaExclusiva=S
VentanaExclusivaOpcion=0
VentanaColor=Plata
Comentarios=CDIVMemGest:CDIVMemGest.Mov+<T> <T>+CDIVMemGest:CDIVMemGest.Movid
[CDIVMemGest]
Estilo=Ficha
Clave=CDIVMemGest
AlineacionAutomatica=S
AcomodarTexto=S
MostrarConteoRegistros=S
Zona=A1
Vista=CDIVMemGest
Fuente={Tahoma, 8, Negro, []}
CarpetaVisible=S

CampoColorLetras=Negro
CampoColorFondo=Blanco
ListaEnCaptura=CDIVMemGest.Observa
Filtros=S
FiltroPredefinido=S
FiltroNullNombre=(sin clasificar)
FiltroEnOrden=S
FiltroTodoNombre=(Todo)
FiltroAncho=20
FiltroRespetar=S
FiltroTipo=General
FichaEspacioEntreLineas=6
FichaEspacioNombres=100
FichaEspacioNombresAuto=S
FichaNombres=Arriba
FichaAlineacion=Centrado
FichaColorFondo=Plata
FichaAlineacionDerecha=S
;menuLocal=S
FiltroGeneral=CDIVMemGest.ID=<T>{info.id}<T>
[CDIVMemGest.CDIVMemGest.Observa]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.Observa
Editar=N
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=40x8
ColorFondo=Blanco
;ConScroll=N



[Acciones.Cerrar]
Nombre=Cerrar
Boton=23
NombreDesplegar=&Cerrar
EnBarraHerramientas=S
TipoAccion=ventana
ClaveAccion=Cerrar
Activo=S
Visible=S

[CDIVMemGest.Columnas]
Observaciones=265
0=-2
