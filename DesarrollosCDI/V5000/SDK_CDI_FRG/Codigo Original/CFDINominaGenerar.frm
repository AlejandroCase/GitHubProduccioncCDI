...
Expresion=RegistrarSeleccionModuloID(<T>Lista<T>, <T>Nomina.Empresa<T>, <T>Nomina.ID<T>)<BR>ProcesarSQL(<T>EXEC spCFDINominaGenerar :nEstacionTrabajo, 0, :tUsuario, @BanderaVersion = 0<T>, EstacionTrabajo, Usuario)
...
Expresion=RegistrarSeleccionModuloID(<T>Lista<T>, <T>Nomina.Empresa<T>, <T>Nomina.ID<T>)<BR>Asigna(Temp.logico,SQL(<T>SELECT NominaEditarFechaPago FROM EmpresaCFD WHERE Empresa =:tEmpresa<T>,Empresa))<BR>SI Temp.Logico<BR>Entonces<BR>EjecutarSQL(<T>EXEC spCFDINominaEditarMov :nEstacionTrabajo, :tEmpresa, :tUsuario<T>, EstacionTrabajo, Empresa, Usuario)<BR>Forma(<T>CFDINominaDatosMov<T>)<BR>Sino<BR>ProcesarSQL(<T>EXEC spCFDINominaGenerar :nEstacionTrabajo, 1, :tUsuario, @BanderaVersion = 0<T>, EstacionTrabajo, Usuario)<BR>Fin<BR>ActualizarVista<BR>ActualizarForma
[Acciones.Recibo]
Nombre=Recibo
Boton=61
NombreEnBoton=S
