...
ActivoCondicion=CP:CP.Estatus EN(<T>SINAFECTAR<T>)
AntesExpresiones=Asigna(Info.ID,CP:CP.ID)
VisibleCondicion=CP:CP.Mov = <T>Borrador<T>
[Acciones.EliminarD.Eli]
Nombre=Eli
Boton=0
TipoAccion=expresion
Activo=S
...
Expresion=EjecutarSQL( <T>EXEC CDICPEliminaDetalleCP :nID<T>,info.ID )
[Acciones.EliminarD.Actu]
Nombre=Actu
Boton=0
TipoAccion=Controles Captura
...
Boton=35
NombreEnBoton=S
NombreDesplegar=&Eliminar Detalle
Multiple=S
EnBarraHerramientas=S
EnMenu=S
ListaAccionesMultiples=(Lista)
...
[Acciones.EliminarD.ListaAccionesMultiples]
(Inicio)=Eli
Eli=Actu
Actu=(Fin)
