...
Expresion=ProcesarSQL(<T>xpCDIBonCompraDet :nid, :tP<T>,Compra:Compra.ID,info.CDICompraProv)<BR>FormaModal(<T>CDIBoniCompras<T>)
[Acciones.Boton.Actualizar Forma]
...
GuardarAntes=S
DespuesGuardar=S
ActivoCondicion=Compra:Compra.Mov en (<T>Solicitud Cargo<T>)
...
Expresion=FormaModal(<T>CDICompraProv<T>)<BR>ASIGNA(Info.Proveedor,Info.Proveedor)
...
[Acciones.Boton.ListaAccionesMultiples]
(Inicio)=id
id=Expresion
