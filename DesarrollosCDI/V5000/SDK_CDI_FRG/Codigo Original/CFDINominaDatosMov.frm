...
Expresion=SI SQL(<T>SELECT 1 FROM CFDINominaDatosMov WHERE Estacion = :nEstacion AND FechaPago IS NULL<T>, EstacionTrabajo) = 1<BR>Entonces<BR> Error( <T>Falta Indicar la Fecha de Pago<T> )<BR>Sino<BR>ProcesarSQL(<T>EXEC spCFDINominaGenerar :nEstacionTrabajo, 1, :tUsuario,  @BanderaVersion = 0<T>, EstacionTrabajo, Usuario)<BR>Fin                                                                    <BR>ActualizarVista<BR>ActualizarForma
