...
[Forma]
Clave=CDIExpAsisteFiltrado
Icono=0
Modulos=(Todos)
Nombre=Checadas por Personal
...
FiltroTodo=S
FiltroValida1=CDIExpAsistePers.Tipo
FiltroTodoFinal=S
FiltroGeneral=CDIExpAsistePers.Tipo<><T>EXTERNO<T><BR>and PersonalEntrada IN (SELECT Personal FROM CDIRepPersonal  WHERE Usuario= <T>{Usuario}<T> )
...
[Acciones.Personal.ListaAccionesMultiples]
(Inicio)=1
1=2
2=3
3=(Fin)
