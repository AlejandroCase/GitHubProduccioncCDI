[Forma]
Clave=ExpCamp1
Icono=0
Modulos=(Todos)
Nombre=Gestion CRM
VentanaTipoMarco=Normal
VentanaPosicionInicial=Centrado
VentanaEscCerrar=S
VentanaEstadoInicial=Normal

ListaCarpetas=(Lista)
CarpetaPrincipal=ExpCamp1
PosicionInicialIzquierda=0
PosicionInicialArriba=12
PosicionInicialAlturaCliente=705
PosicionInicialAncho=1382
PosicionSec1=331
PosicionCol1=1005
MovModulo=(Todos)
SinCondicionDespliege=S
SinTransacciones=S
[ExpCamp1]
Estilo=Hoja
Clave=ExpCamp1
AlineacionAutomatica=S
AcomodarTexto=S
MostrarConteoRegistros=S
Zona=A1
Vista=Campana
Fuente={Tahoma, 8, Negro, []}
HojaTitulos=S
HojaMostrarColumnas=S
HojaMostrarRenglones=S
HojaColoresPorEstatus=S
HojaPermiteInsertar=S
HojaPermiteEliminar=S
HojaVistaOmision=Automática
CampoColorLetras=Negro
CampoColorFondo=Blanco
ListaEnCaptura=(Lista)

CarpetaVisible=S

Filtros=S
FiltroPredefinido=S
FiltroGrupo1=Campana.Agente
FiltroNullNombre=(sin clasificar)
FiltroEnOrden=S
FiltroTodoNombre=(Todo)
FiltroAncho=20
FiltroRespetar=S
FiltroTipo=Múltiple (por Grupos)
FiltroGrupo2=Campana.Mov
FiltroTodo=S
BusquedaRapidaControles=S
FiltroFechas=S
FiltroCambiarPeriodo=S
FiltroBuscarEn=S
FiltroFechasCambiar=S
FiltroFechasCampo=Campana.FechaEmision
FiltroFechasDefault=(Todo)
FiltroFechasNormal=S
FiltroFechasNombre=&Fecha
Pestana=S
PestanaOtroNombre=S
PestanaNombre=General
MenuLocal=S
ListaAcciones=(Lista)
OtroOrden=S
ListaOrden=Campana.ID<TAB>(Decendente)
HojaIndicador=S
HojaOrdenarColumnas=S
;FiltroGeneral=cte.Titular=1
[ExpCamp1.Campana.Mov]
Carpeta=ExpCamp1
Clave=Campana.Mov
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[ExpCamp1.Campana.MovID]
Carpeta=ExpCamp1
Clave=Campana.MovID
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[ExpCamp1.Campana.FechaEmision]
Carpeta=ExpCamp1
Clave=Campana.FechaEmision
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[ExpCamp1.Campana.Usuario]
Carpeta=ExpCamp1
Clave=Campana.Usuario
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Blanco

[ExpCamp1.Campana.Referencia]
Carpeta=ExpCamp1
Clave=Campana.Referencia
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=50
ColorFondo=Blanco

[ExpCamp1.Campana.Agente]
Carpeta=ExpCamp1
Clave=Campana.Agente
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Blanco

[ExpCamp1.Campana.Cajero]
Carpeta=ExpCamp1
Clave=Campana.Cajero
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Blanco

[ExpCamp1.Columnas]
Mov=124
MovID=64
FechaEmision=76
Usuario=126
Referencia=108
Agente=74























Cajero=85

Nombre_1=225
MembresiaInicial=85
MembresiaFinal=80

[v1.CDICampanaGest.Accion]
Carpeta=v1
Clave=CDICampanaGest.Accion
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[v1.CDICampanaGest.Acuerdo]
Carpeta=v1
Clave=CDICampanaGest.Acuerdo
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[v1.CDICampanaGest.Contacto]
Carpeta=v1
Clave=CDICampanaGest.Contacto
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[v1.CDICampanaGest.FechaCompromiso]
Carpeta=v1
Clave=CDICampanaGest.FechaCompromiso
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[v1.CDICampanaGest.FormaPago]
Carpeta=v1
Clave=CDICampanaGest.FormaPago
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco











[v1.Columnas]
Accion=151
Acuerdo=302
Contacto=196
FechaCompromiso=219
FormaPago=278
Tel=64
Telefono=124
Cel=64
Celular=124
TelOf=64
TelOfna=124
Dir=64
Direccion=124
Cor=64
Correo=604



[v2.CDICampanaDCob.MovCxC]
Carpeta=v2
Clave=CDICampanaDCob.MovCxC
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[v2.CDICampanaDCob.MovIdCxC]
Carpeta=v2
Clave=CDICampanaDCob.MovIdCxC
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[v2.CDICampanaDCob.FechaDocto]
Carpeta=v2
Clave=CDICampanaDCob.FechaDocto
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[v2.CDICampanaDCob.Concepto]
Carpeta=v2
Clave=CDICampanaDCob.Concepto
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=50
ColorFondo=Blanco

[v2.CDICampanaDCob.Saldo]
Carpeta=v2
Clave=CDICampanaDCob.Saldo
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[v2.CDICampanaDCob.Procesar]
Carpeta=v2
Clave=CDICampanaDCob.Procesar
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco


[v2.Columnas]
IDMovCXC=64
MovCxC=73
MovIdCxC=72
FechaDocto=94
Concepto=304
Saldo=64
Procesar=40


IdCobroMov=77
IdCobroMovId=105

[V3.CampanaD.Cliente]
Carpeta=V3
Clave=CampanaD.Cliente
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Blanco

[V3.CampanaD.Socio]
Carpeta=V3
Clave=CampanaD.Socio
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=100
ColorFondo=Blanco

[V3.CampanaD.Membresia]
Carpeta=V3
Clave=CampanaD.Membresia
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[V3.Cte.Nombre]
Carpeta=V3
Clave=Cte.Nombre
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=100
ColorFondo=Blanco



[V3.Columnas]
Cliente=55
Socio=57
Membresia=58
Nombre_1=527



[cA.CDICampanaDSit.SitPreviav]
Carpeta=cA
Clave=CDICampanaDSit.SitPreviav
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Blanco

[cA.CDICampanaDSit.SitNueva]
Carpeta=cA
Clave=CDICampanaDSit.SitNueva
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Blanco



[cA.Columnas]
SitPreviav=80
SitNueva=81




[ExpCamp1.Agente.Nombre]
Carpeta=ExpCamp1
Clave=Agente.Nombre
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=100
ColorFondo=Blanco

[ExpCamp1.Campana.MembresiaInicial]
Carpeta=ExpCamp1
Clave=Campana.MembresiaInicial
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco








[cA.ListaEnCaptura]
(Inicio)=CDICampanaDSit.SitPreviav
CDICampanaDSit.SitPreviav=CDICampanaDSit.SitNueva
CDICampanaDSit.SitNueva=(Fin)







[v2.CDICampanaDCob.IdCobroMov]
Carpeta=v2
Clave=CDICampanaDCob.IdCobroMov
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[v2.CDICampanaDCob.IdCobroMovId]
Carpeta=v2
Clave=CDICampanaDCob.IdCobroMovId
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco




























[cdixx.Gestor]
Carpeta=cdixx
Clave=Gestor
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Blanco

[cdixx.Tipo]
Carpeta=cdixx
Clave=Tipo
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Blanco

[cdixx.Concepto]
Carpeta=cdixx
Clave=Concepto
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=50
ColorFondo=Blanco

[cdixx.Total]
Carpeta=cdixx
Clave=Total
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco


[cdixx.Columnas]
Gestor=64
Tipo=64
Concepto=101
Total=64


[cdixx.ListaEnCaptura]
(Inicio)=Gestor
Gestor=Tipo
Tipo=Concepto
Concepto=Total
Total=(Fin)


[v1.ListaEnCaptura]
(Inicio)=CDICampanaGest.Accion
CDICampanaGest.Accion=CDICampanaGest.Acuerdo
CDICampanaGest.Acuerdo=CDICampanaGest.Contacto
CDICampanaGest.Contacto=CDICampanaGest.FechaCompromiso
CDICampanaGest.FechaCompromiso=CDICampanaGest.FormaPago
CDICampanaGest.FormaPago=(Fin)



[Acciones.Estadisticas]
Nombre=Estadisticas
Boton=0
NombreDesplegar=Estadisticas
EnMenu=S
TipoAccion=Formas
ClaveAccion=CDIfechEst
Activo=S
Antes=S
Visible=S




EspacioPrevio=S
AntesExpresiones=Asigna(info.Agente,Campana:Campana.Agente)
[ExpCamp1.Campana.MembresiaFinal]
Carpeta=ExpCamp1
Clave=Campana.MembresiaFinal
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco





[v2.ListaEnCaptura]
(Inicio)=CDICampanaDCob.MovCxC
CDICampanaDCob.MovCxC=CDICampanaDCob.MovIdCxC
CDICampanaDCob.MovIdCxC=CDICampanaDCob.FechaDocto
CDICampanaDCob.FechaDocto=CDICampanaDCob.Concepto
CDICampanaDCob.Concepto=CDICampanaDCob.Saldo
CDICampanaDCob.Saldo=CDICampanaDCob.Procesar
CDICampanaDCob.Procesar=CDICampanaDCob.IdCobroMov
CDICampanaDCob.IdCobroMov=CDICampanaDCob.IdCobroMovId
CDICampanaDCob.IdCobroMovId=(Fin)




















[V3.ListaEnCaptura]
(Inicio)=CampanaD.Cliente
CampanaD.Cliente=CampanaD.Socio
CampanaD.Socio=CampanaD.Membresia
CampanaD.Membresia=Cte.Nombre
Cte.Nombre=(Fin)





[CDIVMemGestionD]
Estilo=Hoja
Pestana=S
Clave=CDIVMemGestionD
Detalle=S
AlineacionAutomatica=S
AcomodarTexto=S
MostrarConteoRegistros=S
Zona=B1
Vista=CDIVMemGestionD
Fuente={Tahoma, 8, Negro, []}
VistaMaestra=Campana
LlaveLocal=CDIVMemGestionD.ID
LlaveMaestra=Campana.ID
HojaTitulos=S
HojaMostrarColumnas=S
HojaMostrarRenglones=S
HojaColoresPorEstatus=S
HojaPermiteInsertar=S
HojaPermiteEliminar=S
HojaVistaOmision=Automática
CampoColorLetras=Negro
CampoColorFondo=Blanco
ListaEnCaptura=(Lista)

CarpetaVisible=S

PestanaOtroNombre=S
PestanaNombre=Asignaciones
HojaOrdenarColumnas=S
OtroOrden=S
BusquedaRapidaControles=S
ListaOrden=CDIVMemGestionD.ID<TAB>(Decendente)
FiltroFechas=S
FiltroModificarEstatus=S
FiltroCambiarPeriodo=S
FiltroBuscarEn=S
FiltroFechasCambiar=S
FiltroFechasCampo=CDIVMemGestionD.fechaemision
FiltroFechasDefault=(Todo)
FiltroFechasNormal=S
FiltroFechasNombre=&Fecha
BusquedaRapida=S
BusquedaEnLinea=S
HojaIndicador=S
BusquedaAncho=20
[CDIVMemGestionD.CDIVMemGestionD.mov]
Carpeta=CDIVMemGestionD
Clave=CDIVMemGestionD.mov
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CDIVMemGestionD.CDIVMemGestionD.movid]
Carpeta=CDIVMemGestionD
Clave=CDIVMemGestionD.movid
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CDIVMemGestionD.CDIVMemGestionD.fechaemision]
Carpeta=CDIVMemGestionD
Clave=CDIVMemGestionD.fechaemision
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CDIVMemGestionD.CDIVMemGestionD.estatus]
Carpeta=CDIVMemGestionD
Clave=CDIVMemGestionD.estatus
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=15
ColorFondo=Blanco

[CDIVMemGestionD.CDIVMemGestionD.Observaciones]
Carpeta=CDIVMemGestionD
Clave=CDIVMemGestionD.Observaciones
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=100
ColorFondo=Blanco

[CDIVMemGestionD.CDIVMemGestionD.referencia]
Carpeta=CDIVMemGestionD
Clave=CDIVMemGestionD.referencia
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=50
ColorFondo=Blanco

[CDIVMemGestionD.CDIVMemGestionD.agente]
Carpeta=CDIVMemGestionD
Clave=CDIVMemGestionD.agente
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Blanco


[CDIVMemGestionD.CDIVMemGestionD.Cliente]
Carpeta=CDIVMemGestionD
Clave=CDIVMemGestionD.Cliente
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Blanco

[CDIVMemGestionD.CDIVMemGestionD.socio]
Carpeta=CDIVMemGestionD
Clave=CDIVMemGestionD.socio
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=100
ColorFondo=Blanco


[CDIVMemGestionD.Columnas]
mov=124
movid=124
fechaemision=94
estatus=94
Observaciones=199
referencia=155
agente=64
membresiainicial=64
Cliente=64
socio=64







[CDIVMemGest]
Estilo=Hoja
Pestana=S
PestanaOtroNombre=S
PestanaNombre=Gestión
Clave=CDIVMemGest
AlineacionAutomatica=S
AcomodarTexto=S
MostrarConteoRegistros=S
Zona=B1
Vista=CDIVMemGest
Fuente={Tahoma, 8, Negro, []}
HojaTitulos=S
HojaMostrarColumnas=S
HojaMostrarRenglones=S
HojaColoresPorEstatus=S
HojaPermiteInsertar=S
HojaPermiteEliminar=S
HojaVistaOmision=Automática
CampoColorLetras=Negro
CampoColorFondo=Blanco
ListaEnCaptura=(Lista)

CarpetaVisible=S

HojaOrdenarColumnas=S
OtroOrden=S
ListaOrden=CDIVMemGest.ID<TAB>(Decendente)
BusquedaRapidaControles=S
FiltroFechas=S
FiltroModificarEstatus=S
FiltroCambiarPeriodo=S
FiltroBuscarEn=S
FiltroFechasCambiar=S
FiltroFechasNormal=S
FiltroFechasNombre=&Fecha
BusquedaRapida=S
BusquedaInicializar=S
BusquedaRespetarControles=S
BusquedaAncho=20
BusquedaEnLinea=S
FiltroFechasCampo=CDIVMemGest.fechaemision
FiltroFechasDefault=(Todo)
HojaIndicador=S
MenuLocal=S
ListaAcciones=ObservacionesMM
Detalle=S
VistaMaestra=Campana
LlaveLocal=CDIVMemGest.ID
LlaveMaestra=Campana.ID
[CDIVMemGest.CDIVMemGest.mov]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.mov
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CDIVMemGest.CDIVMemGest.movid]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.movid
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CDIVMemGest.CDIVMemGest.fechaemision]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.fechaemision
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CDIVMemGest.CDIVMemGest.estatus]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.estatus
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=15
ColorFondo=Blanco

[CDIVMemGest.CDIVMemGest.agente]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.agente
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Blanco

[CDIVMemGest.CDIVMemGest.membresiainicial]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.membresiainicial
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CDIVMemGest.CDIVMemGest.Accion]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.Accion
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CDIVMemGest.CDIVMemGest.Acuerdo]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.Acuerdo
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CDIVMemGest.CDIVMemGest.Contacto]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.Contacto
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CDIVMemGest.CDIVMemGest.FechaCompromiso]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.FechaCompromiso
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CDIVMemGest.CDIVMemGest.FormaPago]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.FormaPago
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco











[CDIVMemGest.CDIVMemGest.Referencia]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.Referencia
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=50
ColorFondo=Blanco


[CDIVMemGest.Columnas]
mov=61
movid=67
fechaemision=66
estatus=82
agente=64
membresiainicial=64
Accion=99
Acuerdo=124
Contacto=96
FechaCompromiso=111
FormaPago=124
Observaciones=157
Referencia=183


Mov=61
Movid=67
FechaEmision=66
Estatus=82
Agente=64
MembresiaInicial=64
Observa=304
[CDIVMemGestionc]
Estilo=Hoja
Pestana=S
PestanaOtroNombre=S
PestanaNombre=Cobro Gestión
Clave=CDIVMemGestionc
Detalle=S
AlineacionAutomatica=S
AcomodarTexto=S
MostrarConteoRegistros=S
Zona=B1
Vista=CDIVMemGestionc
Fuente={Tahoma, 8, Negro, []}
VistaMaestra=Campana
LlaveLocal=CDIVMemGestionc.ID
LlaveMaestra=Campana.ID
HojaTitulos=S
HojaMostrarColumnas=S
HojaMostrarRenglones=S
HojaColoresPorEstatus=S
HojaPermiteInsertar=S
HojaPermiteEliminar=S
HojaVistaOmision=Automática
CampoColorLetras=Negro
CampoColorFondo=Blanco
ListaEnCaptura=(Lista)

CarpetaVisible=S

OtroOrden=S
BusquedaRapidaControles=S
ListaOrden=CDIVMemGestionc.ID<TAB>(Decendente)
FiltroFechas=S
FiltroModificarEstatus=S
FiltroCambiarPeriodo=S
FiltroBuscarEn=S
FiltroFechasCambiar=S
FiltroFechasCampo=CDIVMemGestionc.fechaemision
FiltroFechasDefault=(Todo)
FiltroFechasNormal=S
FiltroFechasNombre=&Fecha
BusquedaRapida=S
BusquedaInicializar=S
BusquedaRespetarControles=S
BusquedaAncho=20
BusquedaEnLinea=S
HojaOrdenarColumnas=S
HojaIndicador=S
[CDIVMemGestionc.CDIVMemGestionc.mov]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.mov
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.movid]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.movid
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.fechaemision]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.fechaemision
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.estatus]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.estatus
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=15
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.agente]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.agente
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=10
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.membresiainicial]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.membresiainicial
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.MovCxC]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.MovCxC
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.MovIdCxC]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.MovIdCxC
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.FechaDocto]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.FechaDocto
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.Concepto]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.Concepto
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=50
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.Saldo]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.Saldo
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.IdCobroMov]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.IdCobroMov
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.IdCobroMovId]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.IdCobroMovId
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=20
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.Observaciones]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.Observaciones
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=100
ColorFondo=Blanco

[CDIVMemGestionc.CDIVMemGestionc.referencia]
Carpeta=CDIVMemGestionc
Clave=CDIVMemGestionc.referencia
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
Tamano=50
ColorFondo=Blanco


[CDIVMemGestionc.Columnas]
mov=91
movid=65
fechaemision=64
estatus=67
agente=64
membresiainicial=64
MovCxC=62
MovIdCxC=77
FechaDocto=62
Concepto=164
Saldo=64
IdCobroMov=64
IdCobroMovId=95
Observaciones=170
referencia=122







[Acciones.Gestion]
Nombre=Gestion
Boton=0
NombreDesplegar=Gestionar
EnMenu=S
TipoAccion=Formas
ClaveAccion=Campana
Activo=S
Visible=S

[CDIVMemGest.CDIVMemGest.Observa]
Carpeta=CDIVMemGest
Clave=CDIVMemGest.Observa
Editar=S
LineaNueva=S
ValidaNombre=S
3D=S
ColorFondo=Blanco


[Acciones.ObservacionesMM]
Nombre=ObservacionesMM
Boton=0
NombreDesplegar=&Observaciones
EnMenu=S
TipoAccion=formas
ClaveAccion=CDIVMemGest
Visible=S

ActivoCondicion=Condatos(CDIVMemGest:CDIVMemGest.ID)
Antes=S
AntesExpresiones=asigna(info.id,CDIVMemGest:CDIVMemGest.ID)


























[CDIVMemGestionD.ListaEnCaptura]
(Inicio)=CDIVMemGestionD.mov
CDIVMemGestionD.mov=CDIVMemGestionD.movid
CDIVMemGestionD.movid=CDIVMemGestionD.fechaemision
CDIVMemGestionD.fechaemision=CDIVMemGestionD.estatus
CDIVMemGestionD.estatus=CDIVMemGestionD.Observaciones
CDIVMemGestionD.Observaciones=CDIVMemGestionD.referencia
CDIVMemGestionD.referencia=CDIVMemGestionD.agente
CDIVMemGestionD.agente=CDIVMemGestionD.Cliente
CDIVMemGestionD.Cliente=CDIVMemGestionD.socio
CDIVMemGestionD.socio=(Fin)

[CDIVMemGest.ListaEnCaptura]
(Inicio)=CDIVMemGest.mov
CDIVMemGest.mov=CDIVMemGest.movid
CDIVMemGest.movid=CDIVMemGest.fechaemision
CDIVMemGest.fechaemision=CDIVMemGest.estatus
CDIVMemGest.estatus=CDIVMemGest.agente
CDIVMemGest.agente=CDIVMemGest.membresiainicial
CDIVMemGest.membresiainicial=CDIVMemGest.Accion
CDIVMemGest.Accion=CDIVMemGest.Acuerdo
CDIVMemGest.Acuerdo=CDIVMemGest.Contacto
CDIVMemGest.Contacto=CDIVMemGest.FechaCompromiso
CDIVMemGest.FechaCompromiso=CDIVMemGest.FormaPago
CDIVMemGest.FormaPago=CDIVMemGest.Observa
CDIVMemGest.Observa=CDIVMemGest.Referencia
CDIVMemGest.Referencia=(Fin)

[CDIVMemGestionc.ListaEnCaptura]
(Inicio)=CDIVMemGestionc.mov
CDIVMemGestionc.mov=CDIVMemGestionc.movid
CDIVMemGestionc.movid=CDIVMemGestionc.fechaemision
CDIVMemGestionc.fechaemision=CDIVMemGestionc.estatus
CDIVMemGestionc.estatus=CDIVMemGestionc.agente
CDIVMemGestionc.agente=CDIVMemGestionc.membresiainicial
CDIVMemGestionc.membresiainicial=CDIVMemGestionc.MovCxC
CDIVMemGestionc.MovCxC=CDIVMemGestionc.MovIdCxC
CDIVMemGestionc.MovIdCxC=CDIVMemGestionc.FechaDocto
CDIVMemGestionc.FechaDocto=CDIVMemGestionc.Concepto
CDIVMemGestionc.Concepto=CDIVMemGestionc.Saldo
CDIVMemGestionc.Saldo=CDIVMemGestionc.IdCobroMov
CDIVMemGestionc.IdCobroMov=CDIVMemGestionc.IdCobroMovId
CDIVMemGestionc.IdCobroMovId=CDIVMemGestionc.Observaciones
CDIVMemGestionc.Observaciones=CDIVMemGestionc.referencia
CDIVMemGestionc.referencia=(Fin)

[ExpCamp1.ListaEnCaptura]
(Inicio)=Campana.Mov
Campana.Mov=Campana.MovID
Campana.MovID=Campana.FechaEmision
Campana.FechaEmision=Campana.Usuario
Campana.Usuario=Campana.Referencia
Campana.Referencia=Campana.Agente
Campana.Agente=Agente.Nombre
Agente.Nombre=Campana.Cajero
Campana.Cajero=Campana.MembresiaInicial
Campana.MembresiaInicial=Campana.MembresiaFinal
Campana.MembresiaFinal=(Fin)

[ExpCamp1.ListaAcciones]
(Inicio)=Gestion
Gestion=Estadisticas
Estadisticas=(Fin)

[Forma.ListaCarpetas]
(Inicio)=ExpCamp1
ExpCamp1=CDIVMemGestionD
CDIVMemGestionD=CDIVMemGest
CDIVMemGest=CDIVMemGestionc
CDIVMemGestionc=(Fin)
