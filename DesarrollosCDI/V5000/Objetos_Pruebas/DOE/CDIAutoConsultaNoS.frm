[Forma]
Clave=CDIAutoConsultaNoS
Nombre=<T>Consulta No. Seguridad Tarjeta Bancaria<T>
Icono=0
Modulos=(Todos)
...
Expresion=Asigna(Temp.Reg, SQL(<T>SELECT u.Usuario, u.Contrasena FROM Usuario u, UsuarioD d WHERE u.Usuario=:tUsuario AND u.Usuario=d.Usuario AND d.Empresa=:tEmp<T>, Info.Usuario, Empresa))<BR>Si<BR>  Temp.Reg[1]=Info.Usuario<BR>Entonces<BR>  Si<BR>    SQL(<T>SELECT Estatus FROM Usuario WHERE Usuario=:tUsuario<T>, Info.Usuario)=EstatusAlta<BR>  Entonces<BR>    Si<BR>      (Temp.Reg[2] = Info.Contrasena) o (Temp.Reg[2] = MD5(Info.Contrasena, <T>p<T>))<BR>    Entonces<BR>      ProcesarSQL(<T>EXEC xpCDIConsultaNoSeguridadTarjeta :tEmpresa, :tAccion, :tC, :tNt, :tUsuario, :tIu, :tComentarios, :nI<T>, Empresa, <T>Consultar<T>, Info.cliente, Info.CDICilindro, Usuario, Info.usuario, Info.CDIComentarios, Info.id)<BR>    Sino<BR>      Error(<T>Contraseña Incorrecta<T>)<BR>    Fin<BR>  SiNo<BR>    Error(<T>Usuario Inactivo<T>)<BR>  Fin<BR>Sino<BR>  Error(<T>Usuario Incorrecto<T>)<BR>Fin
[Acciones.Aceptar]
Nombre=Aceptar
Boton=23
