...
Activo=S
Visible=S
Expresion=Asigna(Temp.Reg, SQL(<T>SELECT u.Usuario, u.Contrasena, u.Autorizar, u.AutorizarSeriesLotes FROM Usuario u, UsuarioD d WHERE u.Usuario=:tUsuario AND u.Usuario=d.Usuario AND d.Empresa=:tEmp<T>, Info.Usuario, Empresa))<BR>Si <BR>  Temp.Reg[1]=Info.Usuario<BR>Entonces<BR>  Si <BR>    Temp.Reg[2]=Info.Contrasena<BR>  Entonces<BR>    Si <BR>      Temp.Reg[3] y Temp.Reg[4]<BR>    Entonces<BR>      EjecutarSQL(<T>spSerieLoteAutorizar :tEmpresa, :tSerieLote, :tUsuario<T>, Empresa, Info.SerieLote, Info.Usuario)<BR>    Sino<BR>      Error(<T>El Usuario Indicado No Puede Autorizar<T>)<BR>    Fin<BR>  Sino<BR>    Error(<T>Contraseña Incorrecta<T>)<BR>  Fin<BR>Sino<BR>  Error(<T>Usuario Incorrecto<T>)<BR>Fin
