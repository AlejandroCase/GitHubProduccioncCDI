...
ActivoCondicion=PuedeAfectar(Usuario.Afectar, Usuario.AfectarOtrosMovs, CP:CP.Usuario) y<BR>((CP:CP.Estatus en (EstatusSinAfectar, EstatusPorConfirmar)) o<BR>((CP:CP.Estatus=EstatusPendiente) y <BR>PuedeAvanzarEstatus(<T>CP<T>, CP:CP.Mov, CP:CP.Estatus, FormaSituacion)))
...
Visible=S
TeclaRapida=Alt+F11
Expresion=ReportePantalla(ReporteMovPantalla(<T>CP<T>, CP:CP.Mov, CP:CP.Estatus), CP:CP.ID)
...
EspacioPrevio=S
GuardarAntes=S
Expresion=ReporteImpresora(ReporteMovImpresora(<T>CP<T>, CP:CP.Mov, CP:CP.Estatus), CP:CP.ID)
...
ActivoCondicion=(ConfigModulo(<T>CP<T>,<T>Situaciones<T>)=<T>Si<T>) y Usuario.ModificarSituacion y<BR>PuedeAvanzarSituacion(<T>CP<T>, CP:CP.Mov, CP:CP.Estatus, FormaSituacion)
