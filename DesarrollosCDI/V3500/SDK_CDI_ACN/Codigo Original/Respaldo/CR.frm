...
ActivoCondicion=PuedeAfectar(Usuario.Afectar, Usuario.AfectarOtrosMovs, CR:CR.Usuario) y <BR>(CR:CR.Estatus en (EstatusSinAfectar, EstatusPendiente, EstatusVigente)) y<BR>PuedeAvanzarEstatus(<T>CR<T>, CR:CR.Mov, CR:CR.Estatus, FormaSituacion)
...
Antes=S
Visible=S
ActivoCondicion=(ConfigModulo(<T>CR<T>,<T>Situaciones<T>)=<T>Si<T>) y Usuario.ModificarSituacion y<BR>PuedeAvanzarSituacion(<T>CR<T>, CR:CR.Mov, CR:CR.Estatus, FormaSituacion)
...
HojaSinBorde=S
PermiteEditar=S
FiltroGeneral=FormaValor.FormaTipo IN (SELECT FormaTipo FROM dbo.fnFormaVisibleMovimiento(<T>CR<T>, <T>{CR:CR.Mov}<T>)) AND FormaValor.Aplica=<T>Movimiento<T> AND FormaValor.AplicaClave=<T>CR{CR:CR.ID}<T>
