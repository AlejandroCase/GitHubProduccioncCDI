...
CampoColorFondo=Blanco
ListaEnCaptura=(Lista)
CondicionVisible=MovTipoEn(<T>GES<T>, Gestion:Gestion.Mov, (GES.SRES, GES.SREU, GES.STAR)) o ((MovTipo(<T>GES<T>, Gestion:Gestion.Mov)=GES.MOD) y MovTipoEn(Gestion:Gestion.OrigenTipo, Gestion:Gestion.Origen, (GES.SRES, GES.SREU, GES.STAR)))
...
CondicionVisible=(Gestion:Gestion.Estatus noen (EstatusSinAfectar, EstatusBorrador, EstatusPorConfirmar)) y (MovTipo(<T>GES<T>, Gestion:Gestion.Mov) en (GES.SRES, GES.SREU, GES.STAR, GES.REU, GES.TAR))
...
FiltroGeneral=ID={Info.ID} OR (Gestion.RamaID={Info.ID} AND Gestion.Estatus IN (<T>PENDIENTE<T>, <T>CONCLUIDO<T>))
