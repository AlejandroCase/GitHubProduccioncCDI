...
ConCondicion=S
EjecucionConError=S
Expresion=Si<BR>  Embarque:Embarque.Estatus en (EstatusSinAfectar, EstatusPorConfirmar)<BR>Entonces<BR>  Si(Forma(<T>EmbarqueDArtVentas<T>), Asigna(EmbarqueD:EmbarqueD.MovPorcentaje, Info.Porcentaje))<BR>Sino<BR>  Forma(<T>EmbarqueDArtVentasInfo<T>)<BR>Fin
ActivoCondicion=MovTipo(<T>EMB<T>, Embarque:Embarque.Mov) = EMB.E
EjecucionCondicion=EmbarqueD:EmbarqueMov.Modulo=<T>VTAS<T>
...
[Acciones.EmbarqueParcialLocal]
Nombre=EmbarqueParcialLocal
Boton=0
UsaTeclaRapida=S
TeclaRapida=Ctrl+D
NombreDesplegar=Embarque Parcial
...
CopiarImporte=EmbarqueParcialLocal
EmbarqueParcialLocal=EliminarPartida
EliminarPartida=(Fin)
