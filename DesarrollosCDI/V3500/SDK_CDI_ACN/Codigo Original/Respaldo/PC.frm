...
ActivoCondicion=PuedeAfectar(Usuario.Afectar, Usuario.AfectarOtrosMovs, PC:PC.Usuario) y <BR>(PC:PC.Estatus en (EstatusSinAfectar, EstatusPendiente, EstatusVigente)) y<BR>PuedeAvanzarEstatus(<T>PC<T>, PC:PC.Mov, PC:PC.Estatus, FormaSituacion)
...
Antes=S
Visible=S
ActivoCondicion=(ConfigModulo(<T>PC<T>,<T>Situaciones<T>)=<T>Si<T>) y Usuario.ModificarSituacion y<BR>PuedeAvanzarSituacion(<T>PC<T>, PC:PC.Mov, PC:PC.Estatus, FormaSituacion)
...
HojaSinBorde=S
PermiteEditar=S
FiltroGeneral=FormaValor.FormaTipo IN (SELECT FormaTipo FROM dbo.fnFormaVisibleMovimiento(<T>PC<T>, <T>{PC:PC.Mov}<T>)) AND FormaValor.Aplica=<T>Movimiento<T> AND FormaValor.AplicaClave=<T>PC{PC:PC.ID}<T>
