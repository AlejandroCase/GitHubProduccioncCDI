...
GuardarAntes=S
EnBarraAcciones=S
Expresion=Si<BR>  ConDatos(Info.Codigo)<BR>Entonces<BR>  Asigna(Temp.Reg, SQL(<T>spSendCodigo :tEstacion, :tCodigo<T>, EstacionTrabajo, Info.Codigo))<BR>  Asigna(Ver.Ticket, Ver.Ticket+Temp.Reg[1])<BR>  Asigna(Ver.Totales, TextoEnLista(Temp.Reg[2]))        <BR>  Asigna(Ver.Imagen, <T>C:\Intelisis\Dibujos\Imagenes\Logo.bmp<T>)<BR>  Asigna(Ver.Aviso, <T>Este es un aviso especial !!!<T>)<BR>  Asigna(Ver.Memo, <T>Descripcion Artículo<T>)<BR>  Asigna(Info.Codigo, Nulo)<BR>Fin
