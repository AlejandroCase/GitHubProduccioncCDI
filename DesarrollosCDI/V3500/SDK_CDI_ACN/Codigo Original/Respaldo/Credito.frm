...
Antes=S
Visible=S
ActivoCondicion=(ConfigModulo(<T>CREDI<T>,<T>Situaciones<T>)=<T>Si<T>) y Usuario.ModificarSituacion y<BR>PuedeAvanzarSituacion(<T>CREDI<T>, Credito:Credito.Mov, Credito:Credito.Estatus, FormaSituacion)
...
HojaSinBorde=S
PermiteEditar=S
FiltroGeneral=FormaValor.FormaTipo IN (SELECT FormaTipo FROM dbo.fnFormaVisibleMovimiento(<T>CREDI<T>, <T>{Credito:Credito.Mov}<T>)) AND FormaValor.Aplica=<T>Movimiento<T> AND FormaValor.AplicaClave=<T>CREDI{Credito:Credito.ID}<T>
