...
ActivoCondicion=PuedeAfectar(Usuario.Afectar, Usuario.AfectarOtrosMovs, RSS:RSS.Usuario) y <BR>(RSS:RSS.Estatus en (EstatusSinAfectar, EstatusBorrador, EstatusPorConfirmar, EstatusPendiente))  y<BR>PuedeAvanzarEstatus(<T>RSS<T>, RSS:RSS.Mov, RSS:RSS.Estatus, FormaSituacion)
...
Visible=S
EspacioPrevio=S
ActivoCondicion=(ConfigModulo(<T>RSS<T>,<T>Situaciones<T>)=<T>Si<T>) y Usuario.ModificarSituacion y<BR>PuedeAvanzarSituacion(<T>RSS<T>, RSS:RSS.Mov, RSS:RSS.Estatus, FormaSituacion)
...
HojaSinBorde=S
PermiteEditar=S
FiltroGeneral=FormaValor.FormaTipo IN (SELECT FormaTipo FROM dbo.fnFormaVisibleMovimiento(<T>RSS<T>, <T>{RSS:RSS.Mov}<T>)) AND FormaValor.Aplica=<T>Movimiento<T> AND FormaValor.AplicaClave=<T>RSS{RSS:RSS.ID}<T>
