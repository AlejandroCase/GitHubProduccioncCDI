...
ActivoCondicion=(ConfigModulo(<T>AGENT<T>,<T>Situaciones<T>)=<T>Si<T>) y Usuario.ModificarSituacion y<BR>PuedeAvanzarSituacion(<T>AGENT<T>, Agent:Agent.Mov, Agent:Agent.Estatus, FormaSituacion)
...
HojaSinBorde=S
PermiteEditar=S
FiltroGeneral=FormaValor.FormaTipo IN (SELECT FormaTipo FROM dbo.fnFormaVisibleMovimiento(<T>AGENT<T>, <T>{Agent:Agent.Mov}<T>)) AND FormaValor.Aplica=<T>Movimiento<T> AND FormaValor.AplicaClave=<T>AGENT{Agent:Agent.ID}<T>
