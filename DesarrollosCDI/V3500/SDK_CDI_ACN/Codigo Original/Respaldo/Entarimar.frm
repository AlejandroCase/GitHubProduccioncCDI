...
Visible=S
GuardarAntes=S
Expresion=Si<BR>  Forma(<T>EspecificarTarima<T>)<BR>Entonces<BR>  EjecutarSQL(<T>spEntarimar :nEstacion, :tEmpresa, :tModulo, :nModuloID, :tMov, :tMovID, :tAccion, @Tarima=:tTarima<T>, EstacionTrabajo, Empresa, Info.Modulo, Info.ID, Info.Mov, Info.MovID, <T>ENTARIMAR<T>, Info.Tarima)<BR>  Forma.ActualizarVista(<T>Mov<T>)<BR>  Forma.ActualizarArbol(<T>Tarimas<T>)<BR>  Forma.ActualizarVista(<T>Tarimas<T>)<BR>Fin
