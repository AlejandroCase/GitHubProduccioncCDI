...
FichaColorFondo=Plata
FichaAlineacionDerecha=S
Totalizadores1=Importe<BR>Desc<BR>Sub total<BR>Impuestos<BR>Total<BR>Renglon<BR>Sobre Precio
...
GuardarAntes=S
RefrescarDespues=S
Expresion=Si(General.OFER, EjecutarSQL(<T>spOfertaAplicar :nID<T>, Venta:Venta.ID))
ActivoCondicion=Venta:Venta.Estatus en (EstatusSinAfectar, EstatusPorConfirmar)
...
ActivoCondicion=Si<BR>  PuedeAfectar(Usuario.Cancelar, Usuario.CancelarOtrosMovs, Venta:Venta.Usuario) y ConDatos(Venta:Venta.ID) y ConDatos(Venta:Venta.MovID) y (Vacio(Venta:Venta.CancelacionID) o (MovTipo(<T>VTAS<T>, Venta:Venta.Mov) en (VTAS.D,VTAS.DF,VTAS.B)))<BR>Entonces<BR>  Si<BR>    MovTipoEn(<T>VTAS<T>, Venta:Venta.Mov, (VTAS.C,VTAS.CS,VTAS.R,VTAS.P,VTAS.S,VTAS.VC,VTAS.VCR,VTAS.SD))<BR>  Entonces<BR>    Venta:Venta.Estatus en (EstatusSinAfectar, EstatusSincro, EstatusBorrador, EstatusPorConfirmar, EstatusPendiente, EstatusPorConfirmar)<BR>  Sino<BR>    Venta:Venta.Estatus en (EstatusSinAfectar, EstatusSincro, EstatusBorrador, EstatusPorConfirmar, EstatusPendiente, EstatusProcesar, EstatusRecurrente, EstatusConcluido) <BR>  Fin<BR>Sino<BR>  Falso<BR>Fin
...
Visible=S
GuardarAntes=S
Expresion=ReporteImpresora(ReporteMovImpresora(Info.SubModuloVenta, Venta:Venta.Mov, Venta:Venta.Estatus), Venta:Venta.ID)
...
UsaTeclaRapida=S
TeclaRapida=Alt+F11
Expresion=ReportePantalla(ReporteMovPantalla(Info.SubModuloVenta, Venta:Venta.Mov, Venta:Venta.Estatus), Venta:Venta.ID)
...
DespuesGuardar=S
Activo=S
AntesExpresiones=Asigna(Afectar.Modulo, <T>VTAS<T>)<BR>Asigna(Afectar.ID, Venta:Venta.ID)<BR>Asigna(Afectar.Mov, Venta:Venta.Mov)<BR>Asigna(Afectar.MovID, Venta:Venta.MovID)<BR>Asigna(Info.ID, Venta:Venta.ID)<BR>Asigna(Info.Cliente, Venta:Venta.Cliente)<BR>Asigna(Info.Fecha, Venta:Venta.FechaEmision)<BR>Asigna(Info.Moneda, Venta:Venta.Moneda)<BR>Asigna(Info.Importe, Redondea(Suma(VentaD:Importe)+Suma(VentaD:Impuesto)-Suma(VentaD:DescGlobal)+Suma(VentaD:ImporteSobrePrecio)-(Venta:Venta.AnticiposFacturados+Venta:Venta.Retencion), Config.VentaCobroRedondeoDecimales))<BR>Asigna(Info.Cerrado, Venta:Venta.Estatus<>EstatusSinAfectar)<BR>Asigna(Info.VentaCobroDevolucion, MovTipoEn(<T>VTAS<T>, Venta:Venta.Mov, (VTAS.VP, VTAS.SD, VTAS.D, VTAS.DF)))<BR>Asigna(Info.FormaPago, Vacio(Usuario.DefFormaPago, Config.FormaPagoOmision))
...
Antes=S
ActivoCondicion=(ConfigModulo(<T>VTAS<T>,<T>Situaciones<T>)=<T>Si<T>) y Usuario.ModificarSituacion y<BR>PuedeAvanzarSituacion(<T>VTAS<T>, Venta:Venta.Mov, Venta:Venta.Estatus, FormaSituacion)
...
Antes=S
DespuesGuardar=S
Expresion=Si<BR>  Forma(<T>CxcAnticiposPendientes<T>)<BR>Entonces<BR>  Asigna(Temp.Reg, SQL(<T>SELECT SUM(AnticipoAplicar), SUM(AnticipoAplicar*Impuestos/(Importe+Impuestos)) FROM Cxc WHERE AnticipoAplicaModulo=:tModulo AND AnticipoAplicaID=:nID<T>, Info.Modulo, Info.ID))<BR>  Asigna(Venta:Venta.AnticiposFacturados, Temp.Reg[1])<BR>  Asigna(Venta:Venta.AnticiposImpuestos,  Temp.Reg[2])<BR>Fin
...
EnMenu=S
TipoAccion=Expresion
Expresion=Si(Forma(<T>ComentarioLista<T>), Asigna(Venta:Venta.Comentarios, Temp.Texto))
Activo=S
VisibleCondicion=Venta:Venta.Estatus en (EstatusSinAfectar, EstatusPorConfirmar)
...
VentaD.ExcluirISAN=SubImpuesto2
SubImpuesto2=SubImpuesto1
SubImpuesto1=SubImpuesto3
SubImpuesto3=VentaD.EnviarA
