...
Multiple=S
ConCondicion=S
ListaAccionesMultiples=ChecarUsuario<BR>ChecarPassword<BR>ChecarEmpresa<BR>ChecarSucursal<BR>ChecarMovMover<BR>ChecarXP<BR>Aceptar<BR>Expiracion
GuardarAntes=S
Antes=S
EjecucionCondicion=ConDatos(Acceso:Acceso.Empresa) y ConDatos(Acceso:Acceso.Usuario) y ConDatos(Acceso:Acceso.Sucursal)
AntesExpresiones=Si(Longitud(Acceso:Acceso.Contrasena)<32, Asigna(Acceso:Acceso.Contrasena, MD5(Acceso:Acceso.Contrasena)))
...
Visible=S
ConCondicion=S
EjecucionCondicion=Si <BR>  AsignarAcceso(Acceso:Acceso.Empresa, Acceso:Acceso.Usuario, Acceso:Acceso.Sucursal, Acceso:Usuario.GrupoTrabajo, Acceso:Acceso.FechaTrabajo, Acceso:Acceso.ID)<BR>Entonces<BR>   GuardarCambios <BR>   Asigna(Info.ContMoneda, SQL(<T>SELECT ContMoneda FROM EmpresaCfg WHERE Empresa=:tEmpresa<T>, Acceso:Acceso.Empresa))<BR>   Asigna(Info.Moneda, Info.ContMoneda)<BR>   Asigna(Info.MonedaBase, Info.ContMoneda)<BR>   Verdadero<BR>Sino<BR>  Falso<BR>Fin
...
EjecucionConError=S
Visible=S
EjecucionCondicion=SQL(<T>SELECT dbo.fnContrasenaOk(:tPassword, :tPasswordUsuario)<T>, Acceso:Acceso.Contrasena, Acceso:Usuario.Contrasena)
EjecucionMensaje=//Inc(Temp.Num)<BR>//Si(Temp.Num>5, EjecutarSQL(<T>spBloquearUsuario :tUsuario<T>, Acceso:Acceso.Usuario) <T>Invalid Password, User Blocked<T>, <T>Invalid Password<T>)<BR><T>Invalid Password<T>
...
Activo=S
Visible=S
Expresion=Si<BR>  General.ExpirarContrasenas y (SQL(<T>SELECT ISNULL(ContrasenaNuncaExpira, 0) FROM Usuario WHERE Usuario=:tUsuario<T>, Acceso:Acceso.Usuario)=0)<BR>Entonces<BR>  Si((Ahora-Acceso:Usuario.ContrasenaFecha) > General.ExpirarDias, Forma(<T>AccesoExpirado<T>))<BR>Fin
...
EjecucionConError=S
Visible=S
EjecucionCondicion=SQL(<T>SELECT Estatus FROM Usuario WHERE Usuario=:tUsuario<T>, Acceso:Acceso.Usuario)=EstatusAlta
