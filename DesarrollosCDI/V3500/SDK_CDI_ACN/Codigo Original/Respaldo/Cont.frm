...
Movimientos=
PosicionColumna3=50
PlantillasExcepciones=Cont:Cont.FechaEmision<BR>Cont:Cont.FechaContable<BR>Cont:Cont.OrigenID<BR>Cont:Cont.Origen<BR>Cont:Cont.OrigenTipo<BR>Cont:Cont.FechaCancelacion<BR>Cont:Cont.FechaConclusion
BarraAyudaBold=S
MovTipo=Todos
...
CamposProrrateo=ContD.Debe<BR>ContD.Haber
MensajeProrrateo=Importe
ExcelCarpetaDestino=Detalle
ExcelColumnas=ContD.Cuenta<BR>ContD.SubCuenta<BR>ContD.Concepto<BR>ContD.Debe<BR>ContD.Haber
...
ActivoCondicion=PuedeAfectar(Usuario.Afectar, Usuario.AfectarOtrosMovs, Cont:Cont.Usuario) y <BR>(Cont:Cont.Estatus en (EstatusSinAfectar, EstatusBorrador, EstatusPorConfirmar)) y<BR>PuedeAvanzarEstatus(<T>CONT<T>, Cont:Cont.Mov, Cont:Cont.Estatus, FormaSituacion)
...
Antes=S
Visible=S
ActivoCondicion=(ConfigModulo(<T>CONT<T>,<T>Situaciones<T>)=<T>Si<T>) y Usuario.ModificarSituacion y<BR>PuedeAvanzarSituacion(<T>CONT<T>, Cont:Cont.Mov, Cont:Cont.Estatus, FormaSituacion)
...
ListaParametros1=ContD:ContD.Cuenta<BR>ContD:Cta.Descripcion<BR>ContD:ContD.Periodo<BR>ContD:ContD.Debe
ListaParametros=S
Visible=S
EspacioPrevio=S
ConCondicion=S
