...
ActivoCondicion=PuedeAfectar(Usuario.Afectar, Usuario.AfectarOtrosMovs, Vale:Vale.Usuario) y <BR>(Vale:Vale.Estatus en (EstatusSinAfectar, EstatusPendiente, EstatusVigente)) y<BR>PuedeAvanzarEstatus(<T>VALE<T>, Vale:Vale.Mov, Vale:Vale.Estatus, FormaSituacion)
...
Antes=S
Visible=S
ActivoCondicion=(ConfigModulo(<T>VALE<T>,<T>Situaciones<T>)=<T>Si<T>) y Usuario.ModificarSituacion y<BR>PuedeAvanzarSituacion(<T>VALE<T>, Vale:Vale.Mov, Vale:Vale.Estatus, FormaSituacion)
...
HojaSinBorde=S
PermiteEditar=S
FiltroGeneral=FormaValor.FormaTipo IN (SELECT FormaTipo FROM dbo.fnFormaVisibleMovimiento(<T>VALE<T>, <T>{Vale:Vale.Mov}<T>)) AND FormaValor.Aplica=<T>Movimiento<T> AND FormaValor.AplicaClave=<T>VALE{Vale:Vale.ID}<T>
