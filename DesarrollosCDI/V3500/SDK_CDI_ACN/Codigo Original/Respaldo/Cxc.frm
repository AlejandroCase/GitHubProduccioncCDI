...
FiltroGeneral={Si(SQL(<T>SELECT ISNULL(Filtrar, 0) FROM SubModulo WHERE Modulo=:tModulo AND SubModulo=:tSub<T>, <T>CXC<T>, Info.SubModuloCxc), <T>Cxc.SubModulo=<T>+Comillas(Info.SubModuloCxc), <T>1=1<T>)}<BR>{Si(ConDatos(Filtro.Mov), <T> AND Cxc.Mov=<T>+Comillas(Filtro.Mov), <T><T>)}<BR>{Si(ConDatos(Filtro.Cliente), <T> AND Cxc.Cliente=<T>+Comillas(Filtro.Cliente), <T><T>)}
