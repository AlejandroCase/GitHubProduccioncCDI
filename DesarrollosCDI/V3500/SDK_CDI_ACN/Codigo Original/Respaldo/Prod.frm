...
DespuesGuardar=S
Visible=S
Expresion=Si <BR>  ProdD:Art.Tipo=TipoVIN<BR>Entonces<BR>  Asigna(Info.Articulo, ProdD:ProdD.Articulo)<BR>  Si(Prod:Prod.Estatus=EstatusSinAfectar, Forma(<T>VINMovMultiple<T>), Forma(<T>VINMovMultipleInfo<T>))<BR>Sino<BR>  Asigna(Info.SumaSerieLote, Nulo)<BR>  Si(Prod:Prod.Estatus=EstatusSinAfectar, Forma(<T>SerieLoteMov<T>), Si(Usuario.ModificarSerieLoteProp, Forma(<T>SerieLoteMovProp<T>), Forma(<T>SerieLoteMovInfo<T>)))<BR>  Si<BR>    (no Config.MultiUnidades) y ConDatos(Info.SumaSerieLote) y (Info.SumaSerieLote<>ABS(ProdD:ProdD.Cantidad)) y (Prod:Prod.Estatus en (EstatusSinAfectar, EstatusPorConfirmar))<BR>  Entonces<BR>    Si<BR>      Confirmacion(<T>La Cantidad Difiere con las Partidas.<T>+NuevaLinea+NuevaLinea+<T>¿ Desea Actualizar la Cantidad ?<T>,BotonSi, BotonNo)=BotonSi)<BR>    Entonces<BR>      Asigna(ProdD:ProdD.Cantidad, Si(Vacio(ProdD:ProdD.Cantidad, 0.0)<0,-Info.SumaSerieLote, Info.SumaSerieLote))<BR>    Fin<BR>  Fin<BR>Fin
ActivoCondicion=Config.SeriesLotesMayoreo y MovTipoEn(<T>PROD<T>, Prod:Prod.Mov, (PROD.E))
...
Antes=S
Visible=S
ActivoCondicion=(ConfigModulo(<T>PROD<T>,<T>Situaciones<T>)=<T>Si<T>) y Usuario.ModificarSituacion y<BR>PuedeAvanzarSituacion(<T>PROD<T>, Prod:Prod.Mov, Prod:Prod.Estatus, FormaSituacion)
