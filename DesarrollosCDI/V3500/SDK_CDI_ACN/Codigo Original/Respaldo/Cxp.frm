...
FiltroGeneral={Si(ConDatos(Filtro.Mov), <T>Cxp.Mov=<T>+Comillas(Filtro.Mov), <T>1=1<T>)}<BR>{Si(ConDatos(Filtro.Proveedor), <T> AND Cxp.Proveedor=<T>+Comillas(Filtro.Proveedor), <T><T>)}
...
ActivoCondicion=(ConfigModulo(<T>CXP<T>,<T>Situaciones<T>)=<T>Si<T>) y Usuario.ModificarSituacion y<BR>PuedeAvanzarSituacion(<T>CXP<T>, Cxp:Cxp.Mov, Cxp:Cxp.Estatus, FormaSituacion)
...
CondicionVisible=(Cxp:Cxp.Estatus = EstatusConcluido) o (Cxp:Cxp.Estatus = EstatusCancelado)
...
DineroDepositosAnticipados=InfoCtaDinero
