...
FiltroGeneral=Cte.Tipo<><T>Estructura<T><BR>{Si(no Usuario.ConsultarClientesOtrosAgentes, <T> AND Cte.Agente=<T>+Comillas(Usuario.DefAgente), <T><T>)}<BR>{Si<BR>   Info.VicClaseContrato = <T>SOCIO<T><BR>Entonces<BR>    <T> AND Cte.EsProveedor=1<T><BR>Fin}
[Lista.Cte.Nombre]
Carpeta=Lista
Clave=Cte.Nombre
Editar=S
