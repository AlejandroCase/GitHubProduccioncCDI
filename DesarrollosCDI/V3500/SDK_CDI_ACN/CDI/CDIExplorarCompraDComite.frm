...
[Forma]
Clave=CDIExplorarCompraDComite
Icono=0
Modulos=(Todos)
...
FiltroGeneral=Compra.Empresa=<T>{Empresa}<T> AND<BR>Compra.Mov IN ({MovListaSQL(<T>COMS<T>, COMS.F, COMS.FL, COMS.EG, COMS.EI, COMS.D, COMS.B,COMS.C,COMS.R )})<BR>and Compra.Proyecto = <T>{Usuario.DefProyecto}<T>
