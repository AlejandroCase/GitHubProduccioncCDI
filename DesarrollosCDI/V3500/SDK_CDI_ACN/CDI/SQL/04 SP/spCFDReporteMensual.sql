...
SET CONCAT_NULL_YIELDS_NULL Off
  ...
  SELECT Empresa, MovID, dbo.fnLimpiarRFC(RFC), Serie, CONVERT(varchar, Folio), CONVERT(varchar, Aprobacion), CONVERT(varchar, Fecha, 103), CONVERT(varchar, CONVERT(DECIMAL(20,2),ISNULL(Importe,0.0)+ISNULL(Impuesto1,0.0)+ISNULL(Impuesto2,0.0)-ISNULL(Retencion1,0.0)-ISNULL(Retencion2,0.0))), CONVERT(varchar, CONVERT(DECIMAL(20,2),Impuesto1)),
  ...
  ELSE 
    ...
    SELECT 'Dato' = '|'+RTRIM(LTRIM(RFC))+'|'+RTRIM(LTRIM(Serie))+'|'+CONVERT(varchar, Folio)+'|'+CONVERT(varchar, Aprobacion)+'|'+CONVERT(varchar, Fecha, 103)+' '+CONVERT(varchar, Fecha, 108)+'|'+CONVERT(varchar, CONVERT(DECIMAL(20,2),ISNULL(Importe,0.0)+ISNULL(Impuesto1,0.0)+ISNULL(Impuesto2,0.0)-ISNULL(Retencion1,0.0)-ISNULL(Retencion2,0.0)*ISNULL(TipoCambio,1)))+'|'+CONVERT(varchar, CONVERT(DECIMAL(20,2),Impuesto1*ISNULL(TipoCambio,1)))+'|1|'
            +dbo.fnCFDtipoComprobante(Modulo, ModuloID)+'|'+dbo.fnCFDDatosPedimento(Modulo, ModuloId, 'PEDIMENTO')+'|'+dbo.fnCFDDatosPedimento(Modulo, ModuloId, 'PEDIMENTOFECHA')+'|'+dbo.fnCFDDatosPedimento(Modulo, ModuloId, 'ADUANA')+'|'
      FROM CFD
     WHERE Empresa = @Empresa AND YEAR(Fecha) = @Ejercicio AND MONTH(Fecha) = @Periodo AND Folio IS NOT NULL
     UNION ALL
    SELECT 'Dato' = '|'+RTRIM(LTRIM(RFC))+'|'+RTRIM(LTRIM(Serie))+'|'+CONVERT(varchar, Folio)+'|'+CONVERT(varchar, Aprobacion)+'|'+CONVERT(varchar, Fecha, 103)+' '+CONVERT(varchar, Fecha, 108)+'|'+CONVERT(varchar, CONVERT(DECIMAL(20,2),ISNULL(Importe,0.0)+ISNULL(Impuesto1,0.0)+ISNULL(Impuesto2,0.0)-ISNULL(Retencion1,0.0)-ISNULL(Retencion2,0.0)*ISNULL(TipoCambio,1)))+'|'+CONVERT(varchar, ROUND(Impuesto1*ISNULL(TipoCambio,1), 2))+'|0|'
            +dbo.fnCFDtipoComprobante(Modulo, ModuloID)+'|'+dbo.fnCFDDatosPedimento(Modulo, ModuloId, 'PEDIMENTO')+'|'+dbo.fnCFDDatosPedimento(Modulo, ModuloId, 'PEDIMENTOFECHA')+'|'+dbo.fnCFDDatosPedimento(Modulo, ModuloId, 'ADUANA')+'|'
      FROM CFD
