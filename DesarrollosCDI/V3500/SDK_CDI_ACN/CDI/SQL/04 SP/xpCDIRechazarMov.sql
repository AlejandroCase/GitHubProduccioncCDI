/********************* xpCDIRechazarMov ***************************/
IF EXISTS(SELECT * FROM sysobjects s WHERE TYPE = 'P' AND NAME = 'xpCDIRechazarMov') DROP PROCEDURE dbo.xpCDIRechazarMov
GO
CREATE PROCEDURE xpCDIRechazarMov
        @Empresa                VARCHAR(5),
        @Sucursal               INT,
        @Modulo                 VARCHAR(5),
        @ID                     INT,
...
--  EXEC spExtraerFecha @FechaRegistro OUTPUT 
  ...
  SELECT TOP 1 @UsuarioAutorizar = cf.Usuario, @Orden = cf.Orden
  ...
  IF @Usuario <> @UsuarioAutorizar AND @UsuarioAutorizar <> @UsuarioGrupo
    SELECT @Ok = 46130 ELSE-- SELECT * FROM MensajeLista WHERE Descripcion LIKE '%autorizar%'
  IF @CDIEstado = 'RECHAZADO'
    SELECT @Ok = 60010, @OkRef = 'El Movimiento ya fue Rechazado'
    
  IF @Ok IS NULL
  BEGIN
  	UPDATE CDIMovFAUsuario SET Rechazado = 1, UsuarioAutoriza = @Usuario, FechaAutoriza = @FechaRegistro
  	 WHERE ID = @ID AND Mov = @Mov AND Modulo = @Modulo AND Usuario IN (@UsuarioGrupo, @Usuario) AND Orden = @Orden
  	 
    SELECT @UsuarioSiguiente = @Usuario, @CDIEstadoN = 'RECHAZADO'
    ...
    SELECT @SQL = 'UPDATE ' + @TablaModulo + ' SET Usuario = @Usuario, CDIEstado =  @CDIEstadoN WHERE ID = @ID'
  
    EXEC sp_ExecuteSQL @SQL, @Variables, @ID, @UsuarioSiguiente, @CDIEstadoN
  ...
  ELSE
    SELECT 'El movimiento fue rechazado'
