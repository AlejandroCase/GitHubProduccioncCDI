...
AS BEGIN 
  ...
  BEGIN
    SELECT @CDICancMovOp = CDICancMovOp, @CDIProcMismoMes = CDIProcMismoMes FROM MovTipo WHERE Modulo = @Modulo AND Mov = @Mov
    SELECT @CPMov = CPOperacion FROM EmpresaCfgMov WHERE Empresa = @Empresa
    ...
    DECLARE crCP CURSOR LOCAL FOR 
     SELECT ID, Mov, MovID
      ...
      WHERE Empresa = @Empresa AND Estatus = 'CONCLUIDO' AND OrigenTipo = @Modulo AND Origen = @Mov AND OrigenID = @MovID
            --AND Mov <> CASE WHEN ISNULL(@CDICancMovOp, 0) = 0 THEN '' ELSE CASE WHEN @CDIProcMismoMes = 1 THEN '' ELSE @CPMov END END  -- Si tiene el check, no cancela mov Operacion
      ORDER BY ID
    ...
    --IF @CDICancMovOp = 1 AND ISNULL(@CDIProcMismoMes, 0) = 0
    --BEGIN
    --  SELECT @IDO = ID 
    --   FROM CP
    --  WHERE Empresa = @Empresa AND Estatus = 'CONCLUIDO' AND OrigenTipo = @Modulo AND Origen = @Mov AND OrigenID = @MovID
    ...
    --  INSERT CP(Sucursal, Empresa, Usuario, Mov, Moneda, TipoCambio, FechaEmision, Proyecto, UEN, Estatus, Observaciones, OrigenTipo, Origen, OrigenID)
    --  SELECT Sucursal, Empresa, @Usuario, Mov, Moneda, TipoCambio, @FechaEmision, Proyecto, UEN, 'SINAFECTAR', 'Envio Mes 13', @Modulo,    @Mov,   @MovID
    --   FROM CP
    --  WHERE ID = @IDO
    ...
    --  IF @CPID IS NOT NULL
    --  BEGIN
    --    INSERT CPD(ID, Sucursal,  Renglon, Tipo, ClavePresupuestal, Comprometido, Sobrante)
    --    SELECT @CPID, @Sucursal, Renglon, Tipo, ClavePresupuestal, ABS(Comprometido) * -1, ABS(Comprometido)
    --    FROM CPD
    --    WHERE ID = @IDO
  ...
  BEGIN    
    ...
    DECLARE @CPD TABLE (
      ...
      ImporteTotal		AS		ISNULL(Importe, 0.0)+ISNULL(Impuesto1, 0.0)+ISNULL(Impuesto2, 0.0)+ISNULL(Impuesto3, 0.0),
    ...
    BEGIN      	    	
      ...
      BEGIN
        SELECT @AplicarCP = mt.AfectarCP,
               ...
               @AplicaImporteTotal = ISNULL(g.Importe, 0.0)+ISNULL(g.Impuestos, 0.0),
      ...
      BEGIN
      	...
      	SELECT @AplicaFactor = @AplicaImporteTotal / NULLIF(ISNULL(Importe, 0.0)+ISNULL(Impuestos, 0.0), 0.0) FROM Gasto WHERE ID = @IDAplica
        INSERT @CPD (
               ClavePresupuestal,   ClavePresupuestalImpuesto1,   Importe,                           Impuesto1,                           AfectarCP,  AplicarCP,  Aplica,  AplicaID)
        SELECT d.ClavePresupuestal, c.ClavePresupuestalImpuesto1, SUM(d.ImporteLinea*@AplicaFactor), SUM(d.ImpuestosLinea*@AplicaFactor), @AfectarCP, @AplicarCP, @Aplica, @AplicaID
          FROM GastoT d
          LEFT OUTER JOIN Concepto c ON c.Modulo = @Modulo AND c.Concepto = d.Concepto
         WHERE d.ID = @IDAplica
         GROUP BY d.ClavePresupuestal, c.ClavePresupuestalImpuesto1
         ORDER BY d.ClavePresupuestal, c.ClavePresupuestalImpuesto1
      END ELSE
        INSERT @CPD (
               ClavePresupuestal,   ClavePresupuestalImpuesto1,   Importe,             Impuesto1,             AfectarCP,  AplicarCP,  Aplica,  AplicaID)
        SELECT d.ClavePresupuestal, c.ClavePresupuestalImpuesto1, SUM(d.ImporteLinea), SUM(d.ImpuestosLinea), @AfectarCP, @AplicarCP, @Aplica, @AplicaID
          FROM GastoT d
          LEFT OUTER JOIN Concepto c ON c.Modulo = @Modulo AND c.Concepto = d.Concepto
         WHERE d.ID = @ID      
         GROUP BY d.ClavePresupuestal, c.ClavePresupuestalImpuesto1
    ...
    BEGIN     	      	
      ...
      BEGIN
        ...
        UPDATE @CPD
           SET Impuesto1 = NULL, Impuesto2 = NULL, Impuesto3 = NULL
      END
...
--select @ok = 1
      ...
      IF @Ok IS NULL
      BEGIN
      	UPDATE CPD SET Importe = ROUND(Importe, 2), Presupuesto = ROUND(Presupuesto, 2), Comprometido = ROUND(Comprometido, 2), Comprometido2 = ROUND(Comprometido2, 2), Devengado = ROUND(Devengado, 2), Devengado2 = ROUND(Devengado2, 2), Ejercido = ROUND(Ejercido, 2), EjercidoPagado = ROUND(EjercidoPagado, 2), Sobrante = ROUND(Sobrante, 2), RemanenteDisponible = ROUND(RemanenteDisponible, 2), Anticipos = ROUND(Anticipos, 2) 
      	WHERE ID = @CPID
        EXEC spAfectar 'CP', @CPID, 'AFECTAR', @EnSilencio = 1, @Conexion = 1, @Ok = @Ok OUTPUT, @OkRef = @OkRef OUTPUT
      END
      IF @Ok IS NULL OR @Ok BETWEEN 80030 AND 81000
